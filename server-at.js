...
// ─── CONFIG ────────────────────────────────────────────────
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://tpaseny_db_user:tF0gEGhn7Tlpxz79@cluster0.jozvjqr.mongodb.net/pst_telecom?appName=Cluster0';
...
app.get('/api/sensms/users', async (req, res) => {
  try {
    var token = req.headers['authorization'] || req.headers['x-admin-token'] || req.query.token;
...
app.post('/api/sensms/add-credits', async (req, res) => {
  try {
    ...
    const { ObjectId } = require('mongodb');
    try {
      await db.collection('sensms_users').updateOne({ _id: new ObjectId(userId) }, { $inc: { credits: parseInt(credits) } });
    } catch(e2) {
      await db.collection('sensms_users').updateOne({ id: userId }, { $inc: { credits: parseInt(credits) } });
    }
    res.json({ success: true });
...
app.post('/api/sensms/delete-user', async (req, res) => {
  try {
    ...
    try {
      ...
      await db.collection('sensms_users').deleteOne({ id: userId });
    }
    res.json({ success: true });
  } catch(e) { res.status(500).json({ success: false, error: e.message }); }
});
